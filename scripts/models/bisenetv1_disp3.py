...
class DispHead(nn.Module):
    def __init__(self, c1, n_classes, upscale_factor, is_aux=False) -> None:
        ...
        # self.mlp_c = MLPClassificationLite(max_disp=256)#.apply(weights_init)
        # self.mlp_o = MLPOffsetLite()#.apply(weights_init)
        self.mlp_c = MLPClassification(max_disp=256)#.apply(weights_init)
        self.mlp_o = MLPOffset()#.apply(weights_init)

    def upsample_1d(self, x, d1=48, d2=80):
        x = x.reshape(x.size(0), x.size(1), d1, d2)
        x = self.upsample(x)
        x = x.reshape(x.size(0), x.size(1), d1*d2*8*8)
        return x
