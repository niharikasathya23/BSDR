...
class BiSeNetv1Disp2(nn.Module):
    def __init__(self, backbone: str = 'ResNet-18', num_classes: int = 2, using_separation_loss=False) -> None:
        super().__init__()
        ...
        self.logits_head = Head(256, num_classes, upscale_factor=8, is_aux=False)
        self.disp_head = DispHead(256, 1, upscale_factor=8, is_aux=False)
    ...
    def forward(self, x):                                       # 4x3x1024x2048
        ...
        refined = self.disp_head(fm_fuse)
        refined = torch.sigmoid(refined)*2-1 # [-1, 1] range
        ...
        if self.training:
            ...
            if self.using_separation_loss:
                return logits, context32, context_out32, refined
            else:
                return logits, context_out16, context_out32, refined

        return logits, refined


if __name__ == '__main__':
    model = BiSeNetv1Disp('MobileNetV2-1.0', 19)
